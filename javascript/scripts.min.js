...
xhr=Ajax.createXHR();if(xhr!=undefined){xhr.open("POST","http://localhost/sistemaRest/api/v1/tecnico/index.php?a=3",!0);xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');xhr.onreadystatechange=function(){if(xhr.readyState=='4'){var json=JSON.parse(xhr.responseText);var table=document.getElementById("tabela");table.innerHTML="";var len=0;if(json.tecnicos!=null){len=json.tecnicos.length}
...
static confirmar(codigo){xhr=Ajax.createXHR();var ok=window.confirm("Voce tem certeza que deseja excluir?");if(ok&&xhr!=undefined){var mensagem="";if(codigo==""){mensagem+="Código invalido"}else{codigo="&id="+codigo}
var token=Login.getCookie('token');var consulta="";if(token!==""){consulta="&tk="+token}
if(mensagem==""){xhr.open("POST","http://localhost/sistemaRest/api/v1/tecnico/index.php?a=6"+codigo+consulta,!0);xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');xhr.onreadystatechange=function(){if(xhr.readyState=='4'){var json=JSON.parse(xhr.responseText);alert(json.mensagem);location.reload()}}
xhr.send()}else{alert(mensagem)}}}
static cadastrar(form){document.getElementById("mensagem").innerHTML="<br /><b>Aguarde...</b>";xhr=Ajax.createXHR();if(Tecnico.valida()==!1){return !1}
