...
if(xhr!=undefined){xhr.open("POST","http://localhost/sistemaRest/api/v1/controller/divisao.php?a=3"+consulta,!0);xhr.onload=function(e){if(xhr.readyState=='4'){var json=JSON.parse(xhr.responseText);var table=document.getElementById("tabela");table.innerHTML="";var len=0;if(json.divisaos!=null){len=json.divisaos.length}
...
static confirmar(codigo)
{var xhr=Ajax.createXHR();var ok=window.confirm("Voce tem certeza que deseja excluir?");if(ok&&xhr!=undefined){var mensagem="";if(codigo==""){mensagem+="Código invalido"}else{codigo="&id="+codigo}
var token=Login.getCookie('token');var consulta="";if(token!==""){consulta="&tk="+token}
if(mensagem==""){xhr.open("GET","http://localhost/sistemaRest/api/v1/controller/divisao.php?a=6"+codigo+consulta,!0);xhr.setRequestHeader("Content-Type","application/json;charset=UTF-8");xhr.onreadystatechange=function(){if(xhr.readyState=='4'){var json=JSON.parse(xhr.responseText);alert(json.mensagem);location.reload()}}
xhr.send()}else{alert(mensagem)}}}
static cadastrar(form)
{document.getElementById("mensagem").innerHTML="<br /><b>Aguarde...</b>";var xhr=Ajax.createXHR();var mensagem="";if(form.txtNome.value==""){mensagem+="<br /><b>Você não preencheu a divisao</b>"}
var token=Login.getCookie('token');var consulta="";if(token!==""){consulta="&tk="+token}
if(mensagem==""&&xhr!=undefined){xhr.open("POST","http://localhost/sistemaRest/api/v1/controller/divisao.php?a=4"+consulta,!0);xhr.setRequestHeader("Content-Type","application/json;charset=UTF-8");xhr.onreadystatechange=function(){if(xhr.readyState=='4'&&xhr.status=='200'){document.getElementById("txtNome").value="";var json=JSON.parse(xhr.responseText);document.getElementById("mensagem").innerHTML=json.mensagem}}
xhr.send(Divisao.formToJSON(form))}else{document.getElementById("mensagem").innerHTML=mensagem}}
static atualizar(form)
{document.getElementById("mensagem").innerHTML="<br /><b>Aguarde...</b>";var xhr=Ajax.createXHR();var codigo=document.getElementById("codigo").value;var mensagem="";if(codigo==""){mensagem+="Código invalido"}else{codigo="&id="+codigo}
if(document.getElementById("txtNome").value==""){mensagem+="<br /><b>Você não preencheu a divisao</b>"}
var token=Login.getCookie('token');var consulta="";if(token!==""){consulta="&tk="+token}
if(mensagem==""&&xhr!=undefined){xhr.open("POST","http://localhost/sistemaRest/api/v1/controller/divisao.php?a=5"+codigo+consulta,!0);xhr.setRequestHeader("Content-Type","application/json;charset=UTF-8");xhr.onreadystatechange=function(){if(xhr.readyState=='4'&&xhr.status=='200'){var json=JSON.parse(xhr.responseText);document.getElementById("mensagem").innerHTML="<br /><b>"+json.mensagem+"</b>"}}
xhr.send(Divisao.formToJSON(form))}else{document.getElementById("mensagem").innerHTML=mensagem}}};class Tecnico
{static valida()
