...
if(strErro!==""){alert(strErro);return !1}}};let xhr='';class Categoria{static consultar(form){var pesquisa='';if(form!=null&&form.txtNome.value!=undefined){pesquisa=form.txtNome.value}
xhr=Ajax.createXHR();if(xhr!=undefined){xhr.open("POST","http://localhost/sistemaRest/api/v1/categoria/index.php?a=3",!0);xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');xhr.onreadystatechange=function(){if(xhr.readyState=='4'){var json=JSON.parse(xhr.responseText);var table=document.getElementById("tabela");table.innerHTML="";var len=0;if(json.categorias!=null){len=json.categorias.length}
...
static confirmar(codigo){xhr=Ajax.createXHR();var ok=window.confirm("Voce tem certeza que deseja excluir?");if(ok){document.getElementById("ajax-loader").style.display='block';var mensagem="";if(codigo==""){mensagem+="Código invalido"}else{codigo="&id="+codigo}
var token=Login.getCookie('token');var consulta="";if(token!==""){consulta="&tk="+token}
if(mensagem==""&&xhr!=undefined){xhr.open("POST","http://localhost/sistemaRest/api/v1/categoria/index.php?a=6"+codigo+consulta,!0);xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');xhr.onreadystatechange=function(){if(xhr.readyState=='4'&&xhr.status=='200'){document.getElementById("ajax-loader").style.display='none';location.reload();var json=JSON.parse(xhr.responseText);alert(json.mensagem)}}
xhr.send()}else{alert(mensagem)}}}
static cadastrar(form){document.getElementById("mensagem").innerHTML="<br /><b>Carregando...</b>";xhr=Ajax.createXHR();var mensagem="";if(form.txtNome.value==""){mensagem+="<br /><b>Você não preencheu a categoria</b>"}
var token=Login.getCookie('token');var consulta="";if(token!==""){consulta="&tk="+token}
...
static atualizar(form){document.getElementById("mensagem").innerHTML="<br /><b>Carregando...</b>";xhr=Ajax.createXHR();var codigo=form.codigo.value;var mensagem="";if(codigo==""){mensagem+="Código invalido"}else{codigo="&id="+codigo}
